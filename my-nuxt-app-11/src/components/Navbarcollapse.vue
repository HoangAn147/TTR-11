<template>
  <!-- Offcanvas nav identical to your markup, with only an added id for targeting -->
  <div id="offcanvas-nav" class="navbar-collapse offcanvas offcanvas-nav offcanvas-start" role="dialog" aria-modal="false" aria-hidden="true">
    ...
    <div class="offcanvas-body ms-lg-auto d-flex flex-column h-100">
              <ul class="navbar-nav">
                <li class="nav-item dropdown dropdown-mega">
                  <a class="nav-link dropdown-toggle" href="#" data-bs-toggle="dropdown" aria-expanded="false">Demos</a>
...
let cleanup = () => {}
onMounted(() => {
  const el = document.querySelector('#offcanvas-nav')
  if (!el) return

  const hasBs = typeof window !== 'undefined' && window.bootstrap && window.bootstrap.Offcanvas
  if (hasBs) return

  const burger = document.querySelector('.hamburger.offcanvas-nav-btn, .offcanvas-nav-btn')
  const open = (e) => {
    e && e.preventDefault()
  ...
  el.querySelectorAll('[data-bs-dismiss="offcanvas"], [data-bs-dismiss=offcanvas]').forEach(btn => btn.addEventListener('click', close))
  const onKey = (ev) => { if (ev.key === 'Escape') close() }
  document.addEventListener('keydown', onKey)

  cleanup = () => {
    burger && burger.removeEventListener('click', open)
    el.querySelectorAll('[data-bs-dismiss="offcanvas"], [data-bs-dismiss=offcanvas]').forEach(btn => btn.removeEventListener('click', close))
